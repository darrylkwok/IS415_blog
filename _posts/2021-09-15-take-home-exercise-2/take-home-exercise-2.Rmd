...
```{r}
plot(envelope(bishan_2019_ppp, Lest, nsim = 199))
```
...
```{r}
plot(envelope(jurongw_2019_ppp, Lest, nsim = 99))
```
...
```{r}
plot(envelope(newton_2019_ppp, Lest, nsim = 199))
```
...
Performing Complete Spatial Randomness Test
  ...
  - Null Hypothesis : The distribution of the Airbnb Listings in Woodlands during 2019 are randomly distributed
...
```{r}
plot(envelope(woodlands_2019_ppp, Lest, nsim = 199))
```
...
```{r}
plot((density(split(rescale(june_2019_marked_ppp[sg_owin], 1000)), sigma=bw.ppl)), main="Kernel Density Estimation for 2019 Airbnb Listings")
```

## Kernel Density Estimation for Airbnb Listings in 2021

```{r}
plot((density(split(rescale(june_2021_marked_ppp[sg_owin], 1000)), sigma=bw.ppl)), main="Kernel Density Estimation for 2021 Airbnb Listings")
```
...
plot((density(split(rescale(bishan_2019_marked_ppp, 1000)))), main = 'Bishan')
...
plot((density(split(rescale(woodlands_2019_marked_ppp, 1000)))), main = 'Woodlands')
```
...
plot((density(split(rescale(bishan_2021_marked_ppp, 1000)))), main = 'Bishan')
plot((density(split(rescale(jurongw_2021_marked_ppp, 1000)))), main = 'Jurong West')
plot((density(split(rescale(newton_2021_marked_ppp, 1000)))), main = 'Newton')
plot((density(split(rescale(woodlands_2021_marked_ppp, 1000)))), main = 'Woodlands')
```
...
## Second Order SPPA on Airbnb Listings
...
plot(envelope(bishan_2019_marked_ppp, Lcross, i = "Entire home/apt",
            j = "Private room", correction='border', nsim=999), xlab="distance(m)")
```

Compute Cross L-function for Entire home/apt and Shared room
...
Perform CSR Testing using Cross L-function for Entire home/apt and Shared room

```{r}
plot(envelope(bishan_2019_marked_ppp, Lcross, i = "Entire home/apt",
            j = "Shared room", correction='border', nsim=999), xlab="distance(m)")
```
...
```{r}
plot(envelope(bishan_2019_marked_ppp, Lcross, i = "Private room",
            j = "Shared room", correction='border', nsim=999), xlab="distance(m)")
```
...
```{r}
plot(envelope(jurongw_2019_marked_ppp, Lcross, i = "Entire home/apt",
            j = "Private room", correction='border', nsim=999), xlab="distance(m)")
```
